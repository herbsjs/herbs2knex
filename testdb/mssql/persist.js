...
describe('Persist Entity', () => {
    ...
    describe('Simple entity', () => {
        ...
        it('should insert a new item', async () => {
            ...
            //then
            const retDB = await pool.query(`SELECT id FROM ${database}..${table} WHERE id = ${aModifiedInstance.id}`)
            assert.deepStrictEqual(ret, true)
            assert.deepStrictEqual(retDB.recordset[0].id, 1)
        })
        ...
        it('should update an existing item', async () => {
            ...
            const aModifiedInstance = givenAnModifiedEntity()
            /* clean table for this ID */
            await pool.query(`DELETE FROM ${database}..${table} WHERE id = ${aModifiedInstance.id}`)
